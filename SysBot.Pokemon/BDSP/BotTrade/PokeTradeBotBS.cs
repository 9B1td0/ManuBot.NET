...
public class PokeTradeBotBS(PokeTradeHub<PB8> Hub, PokeBotState Config) : PokeRoutineExecutor8BS(Config), ICountBot
{
    ...
    private async Task HandleAbortedTrade(PokeTradeDetail<PB8> detail, PokeRoutineType type, uint priority, PokeTradeResult result)
    {
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8BS sav, PokeTradeDetail<PB8> poke, CancellationToken token)
    {
        ...
        var trainerNID = GetFakeNID(tradePartner.OT, tradePartner.TrainerID);
        RecordUtil<PokeTradeBotBS>.Record($"Initiating\t{trainerNID:X16}\t{tradePartner.OT}\t{poke.Trainer.TrainerName}\t{poke.Trainer.ID}\t{poke.Id}\t{toSend.EncryptionConstant:X8}");
        Log($"Found Link Trade partner: {tradePartner.OT}-{tradePartner.TID7} (ID: {trainerNID}");
        ...
        if (Hub.Config.Trade.UseTradePartnerDetails && TradeExtensions<PB8>.TrySetPartnerDetails(this, tradePartner, poke, Hub.Config, out var toSendEdited))
        {
            toSend = toSendEdited;
            await SetBoxPokemonAbsolute(BoxStartOffset, toSend, token, sav).ConfigureAwait(false);
        }
