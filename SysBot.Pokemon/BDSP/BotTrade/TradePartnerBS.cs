...
public sealed class TradePartnerBS : ITradePartner
{
    ...
    public static string ReadStringFromRAMObject(ReadOnlySpan<byte> obj)
    {
        ...
        int length = ReadInt32LittleEndian(obj[ofs_len..]);
        if (length < 0 || length > maxCharCount)
            length = maxCharCount;

        return StringConverter8.GetString(obj.Slice(ofs_chars, length * 2));
    }
}
