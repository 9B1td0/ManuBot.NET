...
public class PokeTradeBotLA(PokeTradeHub<PA8> Hub, PokeBotState Config) : PokeRoutineExecutor8LA(Config), ICountBot
{
    ...
    private async Task HandleAbortedTrade(PokeTradeDetail<PA8> detail, PokeRoutineType type, uint priority, PokeTradeResult result)
    {
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8LA sav, PokeTradeDetail<PA8> poke, CancellationToken token)
    {
        ...
        RecordUtil<PokeTradeBotLA>.Record($"Initiating\t{trainerNID:X16}\t{tradePartner.OT}\t{poke.Trainer.TrainerName}\t{poke.Trainer.ID}\t{poke.ID}\t{toSend.EncryptionConstant:X8}");
        Log($"Found Link Trade partner: {tradePartner.OT}-{tradePartner.TID7:000000} (ID: {trainerNID})");

        var partnerCheck = await CheckPartnerReputation(this, poke, trainerNID, tradePartner.OT, AbuseSettings, token).ConfigureAwait(false);
        ...
        if (Hub.Config.Trade.UseTradePartnerDetails && TradeExtensions<PA8>.TrySetPartnerDetails(this, tradePartner, poke, Hub.Config, out var toSendEdited))
        {
            toSend = toSendEdited;
            await SetBoxPokemonAbsolute(BoxStartOffset, toSend, token, sav).ConfigureAwait(false);
        }
    ...
    protected virtual async Task<bool> WaitForTradePartner(CancellationToken token)
    {
        ...
        while (ctr > 0)
        {
            ...
            if (ReadInt32LittleEndian(data) != 2)
                continue;
    ...
    private async Task<PokeTradeResult> ProcessDumpTradeAsync(PokeTradeDetail<PA8> detail, CancellationToken token)
    {
        ...
        await detail.Notifier.TradeFinished(this, detail, detail.TradeData).ConfigureAwait(false); // blank PA8
        return PokeTradeResult.Success;
    }
    ...
    private async Task<TradePartnerLA> GetTradePartnerInfo(CancellationToken token)
    {
        ...
        var info = await SwitchConnection.PointerPeek(4, Offsets.LinkTradePartnerInfoPointer, token).ConfigureAwait(false);
        return new TradePartnerLA(id, name, info);
    }
