...
public class PokeTradeBotSV(PokeTradeHub<PK9> Hub, PokeBotState Config) : PokeRoutineExecutor9SV(Config), ICountBot
{
    ...
    private async Task HandleAbortedTrade(PokeTradeDetail<PK9> detail, PokeRoutineType type, uint priority, PokeTradeResult result)
    {
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV9SV sav, PokeTradeDetail<PK9> poke, CancellationToken token)
    {
        ...
        if (Hub.Config.Trade.UseTradePartnerDetails && TradeExtensions<PK9>.TrySetPartnerDetails(this, tradePartner.MyInfo, poke, Hub.Config, out var toSendEdited))
        {
            toSend = toSendEdited;
            await SetBoxPokemonAbsolute(BoxStartOffset, toSend, token, sav).ConfigureAwait(false);
        }
    ...
    private async Task<PokeTradeResult> ProcessDumpTradeAsync(PokeTradeDetail<PK9> detail, CancellationToken token)
    {
        ...
        await detail.Notifier.TradeFinished(this, detail, detail.TradeData).ConfigureAwait(false); // blank PK9
        return PokeTradeResult.Success;
    }
